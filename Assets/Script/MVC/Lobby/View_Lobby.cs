...
public class View_Lobby : ViewBehaviour
{
    ...
    [Title("TopRight")] public TextMeshProUGUI nextLevelBestItemText;

    [FormerlySerializedAs("nextRewardLevelText")]
    ...
    [Title("CardCatNotifies")] public Transform cardCatNotifyContent;

    [Title("Particle")] public UIParticle expParticle;
    public UIParticle moneyParticle;
    public UIParticle diamondParticle;

    [Title("Red")] public GameObject lobbyDailyQuestRed;
    ...
    public GameObject eventRedPoint;
    public Animator eventRedPointAnimator;
    ...
    public override void Open()
    {
        ...
        for (int i = 0; i < downButtons.Length; i++)
            downButtons[i].DOAnchorPos(downButtonsOrigins[i], 0.25f).From(buildButtonPos).SetEase(Ease.OutBack)
        ...
        // Right
        tip.DOAnchorPosX(-34f, 0.25f).SetEase(Ease.OutBack)
            .From(new Vector2(tip.sizeDelta.x * 2 + 34f, tip.anchoredPosition.y));
    ...
    private void OnTmpLevelChange(object from, object to)
    {
        ...
        DOVirtual.DelayedCall(2f, () => { levelText.text = afterLevel.ToString("00"); });

        float duration = App.system.tutorial.isTutorial ? 0 : 2.5f;

        DOVirtual.DelayedCall(duration, () => { App.system.levelUp.Open(beforeLevel, afterLevel); });
    }

    private void OnTmpDiamondChange(object from, object to)
    {
        int diamond = (int)to;
        DOVirtual.DelayedCall(1.75f, () => { diamondText.text = diamond.ToString(); });
    }

    private void OnTmpMoneyChange(object from, object to)
    {
        int money = (int)to;
        DOVirtual.DelayedCall(1.75f, () => { coinText.text = money.ToString(); });
    }
    ...
    private void CheckRedActivate() // 檢查紅點有沒有在用
    {
        ...
        if (eventRedPoint.activeSelf)
            DOVirtual.DelayedCall(0.35f, () => eventRedPointAnimator.Play(animationName));
    }
}